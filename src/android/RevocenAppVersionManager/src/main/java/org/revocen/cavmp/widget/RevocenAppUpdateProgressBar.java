...
public class RevocenAppUpdateProgressBar extends View {
	...
	private int DEFAULT_BORDER_WIDTH = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1,
			getResources().getDisplayMetrics());
	...
	public RevocenAppUpdateProgressBar(Context context) {
		this(context, null);
	}

	public RevocenAppUpdateProgressBar(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public RevocenAppUpdateProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
		super(context, attrs, defStyleAttr);

		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.RevocenAppUpdateProgressBar);
		try {
			mMax = a.getInt(R.styleable.RevocenAppUpdateProgressBar_max, DEFAULT_MAX);
			mProgress = a.getInt(R.styleable.RevocenAppUpdateProgressBar_progress, DEFAULT_PROGRESS);
			mRadius = (int) a.getDimension(R.styleable.RevocenAppUpdateProgressBar_progressRadius, DEFAULT_RADIUS);
			mProgressColor = a.getColor(R.styleable.RevocenAppUpdateProgressBar_progressColor, DEFAULT_PROGRESS_COLOR);
			mProgressDescColor = a.getColor(R.styleable.RevocenAppUpdateProgressBar_progressDescColor,
					DEFAULT_PROGRESS_DESC_COLOR);
			mBorderWidth = (int) a.getDimension(R.styleable.RevocenAppUpdateProgressBar_borderWidth, DEFAULT_BORDER_WIDTH);
			mBorderColor = a.getColor(R.styleable.RevocenAppUpdateProgressBar_borderColor, DEFAULT_BORDER_COLOR);
			mProgressDesc = a.getString(R.styleable.RevocenAppUpdateProgressBar_progressDesc);
			if (mProgressDesc == null) {
				mProgressDesc  = "";
			}
			mIsShowDesc = a.getBoolean(R.styleable.RevocenAppUpdateProgressBar_isShowDesc, DEFAULT_ISSHOWDESC);
	...
	private void drawProgressDesc(Canvas canvas) {
		...
		String finalProgressDesc = "正在更新 " + mProgressDesc + " " + mProgress + "%";
		mTextPaint.getTextBounds(finalProgressDesc, 0, finalProgressDesc.length(), mTextBounds);

		canvas.drawText(finalProgressDesc, (int) (mWidth / 2.0 - mTextBounds.width() / 2.0 - 5),
